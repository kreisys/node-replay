...
var Matcher, URL, assert, headerUtils, jsStringEscape;
...
Matcher = (function() {
  function Matcher(request, response, settings) {
  ...
  Matcher.prototype.match = function(request) {
    ...
    headers = headerUtils.prune(headers, this.settings.headers);
    ...
    if (Object.keys(this.headers).length !== Object.keys(headers).length) {
      return false;
    }
    _ref = this.headers;
    for (name in _ref) {
  ...
  Matcher.fromMapping = function(host, mapping, settings) {
    var matcher, request;
